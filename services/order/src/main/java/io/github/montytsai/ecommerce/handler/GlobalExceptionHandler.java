...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleFeignException(FeignException exp, HttpServletRequest req) {
        log.error("Feign client error: {}", exp.getMessage());
        HttpStatus status = HttpStatus.resolve(exp.status());
        if (status == null) {
            status = HttpStatus.INTERNAL_SERVER_ERROR;
        }
        return ResponseEntity
                .status(status)
                .body(new ErrorResponse(req.getRequestURI(), "Remote service call failed: " + exp.contentUTF8()));
    }
